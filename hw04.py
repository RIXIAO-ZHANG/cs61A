...
def make_withdraw(balance, password):
    ...
    def helper(amount,input_password):
        nonlocal balance, incorect_lst
        if len(incorect_lst) >= 3:
            return "Your account is locked. Attempts: {0}".format(incorect_lst)
        elif password != input_password:
            incorect_lst.append(input_password)
...
class VendingMachine:
    ...
    """
    "*** YOUR CODE HERE ***"
    def __init__(self,product,price,stock=0,deposit=0):
        self.product = product
        self.price = price
        self.stock = stock
        self.total_amount = deposit
    ...
    def deposit(self,amount):
        if self.stock == 0:
            ...
            return 'Current balance: ${0}'.format(self.total_amount)

    def vend(self):
        if self.stock > 0:
            change = self.total_amount - self.price
            if change < 0:
                return 'You must deposit ${0}'.format(-change) + ' more.'
            else:
                self.total_amount = 0
                self.stock -= 1
                if change > 0:

...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(eval_operator, MacroProcedure):
            return scheme_eval(eval_operator.apply_macro(rest,env), env)
        eval_operants = rest.map(lambda x: scheme_eval(x,env))
        return scheme_apply(eval_operator,eval_operants,env)
        # END PROBLEM 5
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        "*** YOUR CODE HERE ***"
        # create and return a new Frame instance
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        value = expressions.second.map(lambda x: scheme_eval(x, env))
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        lambda_expr = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first,lambda_expr)
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    "*** YOUR CODE HERE ***"
    bindings.map(lambda x: check_form(x,2,2))    #check each bindings using map function
    symbol_scheme_list =  bindings.map(lambda x: x.first)
    check_formals(symbol_scheme_list)
    value_scheme_list = bindings.map(lambda x: scheme_eval(x.second.first,env))
    return env.make_child_frame(symbol_scheme_list,value_scheme_list)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        macro_expr = MacroProcedure(formals, expressions.second, env)
        env.define(target.first, macro_expr)
        return target.first
    else:
        bad_target = target.first if isinstance(target, Pair) else target
        raise SchemeError('non-symbol: {0}'.format(bad_target))
...
def optimize_tail_calls(original_scheme_eval):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        """Non-atomic expressions and in tail context """
        if tail and not scheme_symbolp(expr) and not self_evaluating(expr):
            return Thunk(expr, env)

        """atomic expressions or not in tail context"""
